"""
Threads API Module - Threads API Integration (Meta Graph API)
"""
...
async def publish_thread(user_id, token, text, reply_to_id=None, image_url=None):
          try:
                        async with aiohttp.ClientSession() as s:
                                          payload = {"media_type": "IMAGE" if image_url else "TEXT", "text": text, "access_token": token}
                                          if image_url: payload["image_url"] = image_url
                                                            if reply_to_id: payload["reply_to_id"] = reply_to_id
                                                                              async with s.post(f"{API}/{user_id}/threads", data=payload) as r:
                                                                                                    d = await r.json()
                                                                                                    if "error" in d: return {"success": False, "error": d["error"].get("message","")}
                                                                                                                          cid = d["id"]
...
async def get_user_profile(user_id, token):
          async with aiohttp.ClientSession() as s:
                ...
                async def get_long_lived_token(short_token, app_secret):
                          async with aiohttp.ClientSession() as s:
                                        async with s.get("https://graph.threads.net/access_token", params={"grant_type":"th_exchange_token","client_secret":app_secret,"access_token":short_token}) as r:
                                                          return await r.json()

                                async def refresh_token(token):
                                          async with aiohttp.ClientSession() as s:
                                                        async with s.get("https://graph.threads.net/refresh_access_token", params={"grant_type":"th_refresh_token","access_token":token}) as r:
                                                                          return await r.json()

                                                async def get_publishing_limit(user_id, token):
                                                          async with aiohttp.ClientSession() as s:
                                                                        async with s.get(f"{API}/{user_id}/threads_publishing_limit", params={"fields":"quota_usage,config","access_token":token}) as r:
                                                                                          return await r.json()
